...
def test_summarize_bootstrap_results_filters_nonfinite_bootstrap_samples():
    y_true = np.array([0, 1, 0, 1, 1, 0, 0])
    y_scores = {'model': np.linspace(0.1, 0.9, y_true.size)}

    def parity_metric(values, _scores):
        return float(np.mean(values)) if values.sum() % 2 else np.inf

    results = bootstrap_evaluation(
        y_true,
        y_scores,
        metric_funcs={'parity_metric': parity_metric},
        n_bootstraps=128,
        random_state=3,
    )

    summary, _ = summarize_bootstrap_results(results)
    summary_text = summary['parity_metric']['model']
    assert 'np.float64' not in summary_text
    assert 'inf' not in summary_text.lower()
    assert 'nan' not in summary_text.lower()


def test_plot_bootstrap_curve_handles_empty_y_score_selection():
    y_true = np.array([0, 1, 0, 1])
    y_scores = {'model': np.array([0.2, 0.8, 0.3, 0.7])}

    metric_funcs = {'mean_label': lambda values, scores: float(np.mean(values))}
    curve_funcs = {'identity_curve': lambda values, scores: (np.array([0, 1]), np.array([0, 1]), np.array([0, 1]))}
