...
def _round_and_squeeze(value, decimal_places=None):
    """Round numeric values and convert NumPy scalars to native Python types."""

    array_value = np.asarray(value)
    if decimal_places is not None:
        array_value = np.round(array_value, decimal_places)
...
def _compute_confidence_interval(theta_hat_b, theta_hat, percentile_func, interval,
                                 method, alternative):
    """Return bootstrap confidence interval bounds and the finite mask."""

    theta_hat_b = np.asarray(theta_hat_b)
    finite_mask = np.isfinite(theta_hat_b)

    if finite_mask.any():
        filtered = theta_hat_b[finite_mask]
        ci_l = percentile_func(filtered, interval[0] * 100)
        ci_u = percentile_func(filtered, interval[1] * 100)
    ...
    return ci_l, ci_u, finite_mask, theta_hat_b
...
def analyze_bootstrap_results(bootstrapped_results, metric_func_name,
    ...
    for y_score_key in y_score_names:
        ...
        ci_l, ci_u, finite_mask, theta_hat_b = _compute_confidence_interval(
            theta_hat_b,
            theta_hat,
            percentile_func,
            interval,
        ...
        ci_l_scalar = _round_and_squeeze(ci_l)
        ci_u_scalar = _round_and_squeeze(ci_u)
        ci = (ci_l_scalar, ci_u_scalar)

        is_within_bounds = finite_mask & (ci_l_scalar <= theta_hat_b) & (theta_hat_b <= ci_u_scalar)
...
def summarize_bootstrap_results(bootstrapped_results, confidence_level=0.95, alternative='two-sided', method='basic', decimal_places=3,
    ...
    for metric_func_name, originals in original_metrics.items():
        ...
        for y_score_key, theta_hat in originals.items():
            ...
            ci_l, ci_u, _, _ = _compute_confidence_interval(
                theta_hat_b,
                theta_hat,
                percentile_func,
                interval,
            ...
            ci = (
                _round_and_squeeze(ci_l, decimal_places),
                _round_and_squeeze(ci_u, decimal_places),
            )
            center = _round_and_squeeze(theta_hat, decimal_places)
            mf_summary_results[metric_func_name][y_score_key] = (
                f"{center} ({ci[0]}, {ci[1]})"
            )
    ...
    for metric_func_name, originals in original_compatibility.items():
        ...
        for y_score_pair_key, theta_hat in originals.items():
            ...
            center = _round_and_squeeze(theta_hat, decimal_places)
            cmf_summary_results[metric_func_name][y_score_pair_key] = (
                f"{center} ({ci[0]}, {ci[1]})"
            )
